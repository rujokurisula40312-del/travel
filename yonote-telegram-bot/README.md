...
## Доступ по кнопке Start

В Telegram человек открывает ссылку на бота вида:

```text
https://t.me/YOUR_BOT_USERNAME
```

Telegram покажет кнопку `Start`. После нажатия бот получит команду `/start` и покажет меню.

Если `BOT_ALLOWED_USER_IDS` оставить пустым, бот будет доступен всем, кто перешел по ссылке и нажал `Start`.

Если нужно ограничить доступ только команде, укажите Telegram ID через запятую:
